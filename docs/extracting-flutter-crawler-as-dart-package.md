...
| `flutter analyze` 從 repo root 跑會掃 package test，找不到 `package:test/test.dart`（是子 package 的 dev_dependency） | 主 repo 的 `analysis_options.yaml` 加 `analyzer.exclude: ['packages/**']`；子 package 在 CI 用獨立 step `cd packages/X && dart analyze` |
