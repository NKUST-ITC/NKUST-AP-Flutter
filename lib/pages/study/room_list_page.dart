...
class RoomListPageState extends State<RoomListPage> {
  ...
  Map<String, Map<String, List<Room>>> _groupRoomsByBuildingAndFloor() {
    ...
    for (final Room room in roomData!.data) {
      ...
      grouped.putIfAbsent(building, () => <String, List<Room>>{});
      grouped[building]!.putIfAbsent(floor, () => <Room>[]).add(room);
    }
    ...
    for (final String building in grouped.keys) {
      ...
      sortedResult[building] = <String, List<Room>>{};
  ...
  _RoomInfo _parseRoomInfo(String name) {
    ...
    if (name.startsWith('(')) {
      ...
      if (endIndex > 0) {
        building = name.substring(1, endIndex); // 去掉外層括號
  ...
  Color _getFloorColor(String floor, ColorScheme colorScheme) {
    ...
    final List<Color> floorColors = <Color>[
      Colors.teal,
      Colors.blue,
      Colors.indigo,
      Colors.purple,
